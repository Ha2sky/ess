...
public class HistoryService {
    ...
    public List<ApplyHistory> getApplyList(LocalDate startDate, LocalDate endDate,
        ...
        if (applyType == null || applyType.isEmpty()) {
            applyList = applyHistoryMapper.getAllApplyList(DateUtil.reverseFormatDate(startDate),
                                                           DateUtil.reverseFormatDate(endDate),
                                                           status, empCode);
        } else if (applyType.equals("연장근로")) {
            applyList = applyHistoryMapper.getApplyList(DateUtil.reverseFormatDate(startDate),
                                                        DateUtil.reverseFormatDate(endDate),
                                                        "연장", status, empCode);
            ...
            applyList = applyHistoryMapper.getApplyList2(DateUtil.reverseFormatDate(startDate),
                                                         DateUtil.reverseFormatDate(endDate),
                                                         status, empCode);
        } else {
    ...
    public List<ApplyHistory> setApplyList(List<ApplyHistory> applyList) {
        for (ApplyHistory applyHistory : applyList) {
            ...
            if (applyHistory.getApplyType().equals("연장") || applyHistory.getApplyType().equals("조출연장")) {
                ShiftMaster shift = empCalendarMapper.findShiftByEmpCodeAndDate(applyHistory.getEmpCode(), applyHistory.getTargetDate());
                ...
                shift.setWorkOnHhmm(applyHistory.getStartTime());
                shift.setWorkOffHhmm(applyHistory.getEndTime());
                applyHistory.setHoliday(String.format("%.2f", WorkHoursCalculator.getTotalWorkTime(shift).toMinutes() / 60.0));
